pipeline {
    ...
    environment {
        ...
        APP_REPO_NAME = "berkay542/todo-app"
        APP_NAME = "todo"
    }
    ...
    stages {
        ...
        stage('Build App Docker Image') {
            steps {
                ...
                sh 'docker buildx build --force-rm -t "$ECR_REGISTRY/$APP_REPO_NAME:postgre" -f ./postgresql/dockerfile-postgresql'
                sh 'docker buildx build --force-rm -t "$ECR_REGISTRY/$APP_REPO_NAME:nodejs" -f ./nodejs/dockerfile-nodejs'
                sh 'docker buildx build --force-rm -t "$ECR_REGISTRY/$APP_REPO_NAME:react" -f ./react/dockerfile-react'
                sh 'docker image ls'
            }
        }
        ...
        stage('Deploy the App') {
            steps {
                ...
                ansiblePlaybook credentialsId: 'firstkeymac', disableHostKeyChecking: true, installation: 'ansible', inventory: 'inventory_aws_ec2.yml', playbook: 'docker-project.yml'
             }
        }
